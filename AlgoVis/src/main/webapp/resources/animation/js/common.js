...
function checkLoggedInUser() {
    $.ajax({
        url: "http://algorithmics.comp.nus.edu.sg/~onlinequiz/php/User.php",
        data: {mode: MODE_GET_LOGGED_IN_USER}
    }).done(function(username) {
        if (username.length > 0) {
            $('#login-link').html('Logout <b>' + username + '</b>');
        } else {
...
$(document).ready(function() {
    ...
    $('#login-link').click(function() {
        if ($('#login-link').text() == 'Login') {
            ...
            $.ajax({
                url: "http://algorithmics.comp.nus.edu.sg/~onlinequiz/php/User.php?mode=" + MODE_LOGOUT,
            }).done(function() {
                customAlert("Successfully logged out.");
                checkLoggedInUser();
            });
        }
    });
    /*-------LOG IN AUTHENTICATION-------*/
    $('#login-overlay-content #login-go').click(function(event) {
        event.preventDefault();
        //authenticate
        $.ajax({
            url: "http://algorithmics.comp.nus.edu.sg/~onlinequiz/php/User.php?mode=" + MODE_LOGIN,
            type: "POST",
            data: {username: $('#login-id').val(), password: $('#login-pw').val()}
        }).done(function(passed) {
            passed = parseInt(passed);
            if (passed == 1) {
                $('#login-err').text('');
                $('#login-id').val('user id').css('color', '#aaa');
                $('#login-pw').val('password').attr('type', 'text').css('color', '#aaa');
